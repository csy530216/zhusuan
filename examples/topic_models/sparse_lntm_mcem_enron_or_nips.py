...
if __name__ == "__main__":
    ...
    def _log_prior(observed):
        log_p_eta, _, _, _ = joint_obj(observed)
        return log_p_eta
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(1, epochs + 1):
            ...
            for t in range(iters):
                ...
                # M step
                _, ll, ll_cuda = sess.run(
                    [infer, log_likelihood, log_likelihood_cuda],
                    feed_dict={x_indices: x_batch_indices, x_values: x_batch_values,
                               eta_mean: Eta_mean,
                               eta_logstd: Eta_logstd,
            ...
            time_epoch += time.time()
            print('Epoch {} ({:.1f}s): Perplexity = {:.2f} : {:.2f}, acc = {:.3f}, '
